...
import { UpdateVideoCaption, UploadToBucketToYoutube } from '../bullMQ.service';

@Injectable()
export class UploadCaptionListener implements OnModuleInit {
  constructor(
    @InjectQueue('nexus-caption-upload') private readonly uploadQueue: Queue,
  ...
  private listenToProgressEvents(): void {
    this.uploadQueue.on('progress', (job: Job<UpdateVideoCaption>, progress: number) => {
        console.log('Upload Caption Job Progress:', job.id, progress);
  ...
  private listenToCompletedEvents(): void {
    this.uploadQueue.on('completed', (job: Job<UpdateVideoCaption>) => {
